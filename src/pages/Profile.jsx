    ...
    <div className="space-y-5 animate-fade-up">
      ...
      <div className="card p-5 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h2 className="text-base font-bold text-text-primary">{user?.nombre} {user?.apellido}</h2>
          <p className="text-sm text-text-secondary">{user?.email}</p>
          <p className="text-xs text-text-muted mt-0.5">CC: {user?.numero_documento}</p>
        </div>
        <Button variant="outline" size="sm" onClick={() => setEditing(!editing)}>Editar</Button>
      </div>

      {editing && (
        <div className="card p-5 space-y-4 animate-fade-up">
          <Input label="Nombre"   defaultValue={user?.nombre}   leftIcon={<User className="w-4 h-4" />} />
          <Input label="Apellidos"defaultValue={user?.apellido} />
          <Input label="Email" type="email" defaultValue={user?.email} leftIcon={<Mail className="w-4 h-4" />} />
          <div className="flex gap-3">
            <Button fullWidth onClick={() => { addToast({ type: 'success', title: 'Perfil actualizado' }); setEditing(false) }}>Guardar</Button>
      ...
      <div className="card overflow-hidden">
        {menu.map((item, i) => (
          <button key={item.label} className={cn('w-full flex items-center gap-3 px-4 py-3.5 hover:bg-border-light transition-colors text-left',
