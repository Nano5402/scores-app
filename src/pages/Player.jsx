...
export default function Player() {
  ...
  if (!player) return <p className="text-center py-16 text-sm" style={{ color: 'var(--text-muted)' }}>Jugador no encontrado</p>
  ...
  return (
    <div className="space-y-5 animate-fade-up">
      <div className="flex items-center justify-between">
        <Link to="/tennis" className="flex items-center gap-2 text-sm transition-colors"
          style={{ color: 'var(--text-secondary)' }}>
          <ArrowLeft className="w-4 h-4" /> Tenis
        </Link>
        <button onClick={() => toggleJugador(player)}
      ...
      <div className="card p-5">
        <div className="flex items-center gap-4">
          <div className="w-20 h-20 rounded-full border-2 flex items-center justify-center text-4xl shrink-0"
          ...
          <div className="flex-1 min-w-0">
            ...
            {player.apodo && (
              <p className="text-sm" style={{ color: 'var(--text-muted)' }}>"{player.apodo}"</p>
            )}
            <div className="flex items-center gap-2 mt-1">
              ...
              {player.categoria && (
                <span className="badge-brand">{player.categoria.nombre}</span>
              )}
