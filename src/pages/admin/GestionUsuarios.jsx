    ...
    <div className="space-y-5 animate-fade-up">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="card p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-2xl font-black" style={{ color: 'var(--text-primary)' }}>{admins}</p>
            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>Administradores</p>
          </div>
        </div>
        <div className="card p-4 flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg flex items-center justify-center"
          ...
          <div>
            <p className="text-2xl font-black" style={{ color: 'var(--text-primary)' }}>{miembros}</p>
            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>Miembros</p>
          </div>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          Array(4).fill(0).map((_, i) => (
            <div key={i} className="skeleton h-16 m-3 rounded-lg" />
          ))
        ) : filtered.length === 0 ? (
            ...
            >
              ...
              <div
                ...
                style={{
                  ...
                  border:          u.id === me?.id ? '2px solid var(--color-brand)' : '1px solid var(--border-color)',
                }}
              >
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span className={u.rol === 'admin' ? 'badge-live' : 'badge-atp'}>
                  {u.rol}
                </span>
                <button
                  onClick={() => toggleRol(u)}
                  ...
                  title={u.id === me?.id ? 'No puedes cambiarte el rol' : `Cambiar a ${u.rol === 'admin' ? 'miembro' : 'admin'}`}
                >
                  {u.rol === 'admin'
