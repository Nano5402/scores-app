  ...
  return (
    <div className="space-y-5 animate-fade-up">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => { reset({ ciudad: 'Cucuta' }); setShowForm(true) }}
      ...
      {showForm && (
        <div className="card p-5 animate-fade-up">
          ...
          <form onSubmit={handleSubmit(onSubmitSede)} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="form-group">
              ...
              <input className="form-input" placeholder="Cucuta"
                {...register('ciudad')} />
            </div>
            <div className="sm:col-span-2 form-group">
              <label className="form-label">Dirección</label>
              <input className="form-input" placeholder="Cra 7 # 12-45"
                {...register('direccion')} />
            </div>
            <div className="sm:col-span-2 flex gap-3">
              <Button type="submit" loading={isSubmitting}>Crear sede</Button>
      ...
      {showCancha && (
        <div className="card p-5 animate-fade-up" style={{ borderColor: 'var(--color-brand)' }}>
          ...
          <form onSubmit={handleC(onSubmitCancha)} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="form-group">
              <label className="form-label">Nombre *</label>
              <input className="form-input" placeholder="Cancha 1"
            ...
            <div className="sm:col-span-3 flex gap-3">
              <Button type="submit" loading={isSubmittingC}>Agregar cancha</Button>
              <Button type="button" variant="secondary" onClick={() => setShowCancha(null)}>Cancelar</Button>
            </div>
          </form>
        </div>
      )}
      ...
      {/* Lista de sedes */}
      {loading ? (
        Array(2).fill(0).map((_, i) => <div key={i} className="skeleton h-20 rounded-xl" />)
      ) : sedes.length === 0 ? (
          ...
          <div key={sede.id} className="card overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-4 py-3">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => { resetC({ deporte: 'ambos', superficie: 'Cemento' }); setShowCancha(sede.id) }}
                  className="btn-ghost p-2 text-xs flex items-center gap-1"
                  style={{ color: 'var(--color-brand)' }}>
                  <Plus className="w-3.5 h-3.5" /> Cancha
                </button>
                <button onClick={() => handleDeleteSede(sede)} className="btn-ghost p-2"
                  style={{ color: '#ef4444' }}>
                ...
                {!canchas[sede.id] ? (
                  ...
                  canchas[sede.id].map((c, i) => (
                    <div key={c.id} className="flex items-center gap-3 px-4 py-2.5"
                      style={{ borderBottom: i < canchas[sede.id].length - 1 ? '1px solid var(--border-color)' : 'none',
