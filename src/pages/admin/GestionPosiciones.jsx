  ...
  const calcularPosiciones = async (torneoId) => {
    ...
    try {
      ...
      partidos.forEach((p) => {
        ...
        const nom1 = isPadel ? p.equipo1?.nombre : `${p.jugador1?.nombre || ''} ${p.jugador1?.apellido || ''}`.trim()
        const nom2 = isPadel ? p.equipo2?.nombre : `${p.jugador2?.nombre || ''} ${p.jugador2?.apellido || ''}`.trim()
        ...
        if (p.ganador === 'jugador1') {
          tabla[id1].pg++; tabla[id1].pts += 3
          tabla[id2].pp++
        } else if (p.ganador === 'jugador2') {
          tabla[id2].pg++; tabla[id2].pts += 3
          tabla[id1].pp++
        }
      })

      const sorted = Object.values(tabla).sort((a, b) =>
        b.pts - a.pts || b.pg - a.pg || a.pp - b.pp
      )
      ...
      {loading ? (
        ...
        <div className="card overflow-hidden">
          ...
          >
            ...
            <span className="text-center font-bold" style={{ color: 'var(--color-brand)' }}>PTS</span>
          </div>
          ...
          {posiciones.map((row, i) => (
            <div
              ...
              style={{
                ...
                backgroundColor: row.pos === 1 ? 'rgba(var(--color-brand-rgb), 0.05)' : 'transparent',
              }}
            >
              <span
