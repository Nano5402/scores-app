  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>Equipos de Pádel</h1>
          <p className="text-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>
            {equipos.length} pareja{equipos.length !== 1 ? 's' : ''} registrada{equipos.length !== 1 ? 's' : ''}
          </p>
        </div>
        ...
        <div className="card p-5 animate-fade-up">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              <Input label="Nombre de la pareja *" placeholder="García / López"
      ...
      <div className="card overflow-hidden">
        {loading ? (
          Array(3).fill(0).map((_, i) => <div key={i} className="skeleton h-16 m-3 rounded-lg" />)
        ) : equipos.length === 0 ? (
          ...
          equipos.map((e, i) => (
            <div key={e.id} className="list-row"
              style={{ borderBottom: i < equipos.length - 1 ? '1px solid var(--border-color)' : 'none' }}>
              <div className="flex -space-x-2 shrink-0">
                {[e.jugador1, e.jugador2].map((j, idx) => (
                  <div key={idx} className="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold border-2"
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                <button onClick={() => handleDelete(e)} className="btn-ghost p-2"
                  style={{ color: '#ef4444' }}>
